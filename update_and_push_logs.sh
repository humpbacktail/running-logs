...
# 未コミットの変更がある場合（Untracked filesを含む）、`git pull --rebase` がブロックされるのを防ぐため、
# 新規ファイルは先にステージングしておく（上記ステップ1.5で実施）
if ! git diff-index --quiet HEAD --; then
  # ただし、すでにステージングされているがコミットされていない変更がある場合は、
  # ここでスクリプトを停止してユーザーにコミットを促す
  echo "⚠️ ローカルに未コミットの変更があります（手動変更など）。先に commit か stash してください。"
  exit 1
fi
